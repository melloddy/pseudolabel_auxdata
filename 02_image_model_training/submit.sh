...
data_path=/home/rama.jabal/Melloddy/aux_data/pseudolabel_auxdata/01_datapreperation/images_files/output_files/image_model/matrices/

if type srun > /dev/null 2>&1; then
    srun ./run_sparsechem.sh $train_path $data_path
else
    sh ./run_sparsechem.sh $train_path $data_path
fi
